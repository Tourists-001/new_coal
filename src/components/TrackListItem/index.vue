...
export default {
  ...
  computed: {
    ...mapState('pcSetting', ['subTitleDefault']),
    ...mapGetters({
    ...
    trackClass() {
      ...
      if (!this.playable && this.showUnavailableSongInGreyStyle) { trackClass.push('disable') }
  ...
  methods: {
    ...
    likeThisSong() {
      this.$parent.likeATracks(this.track.id)
    }
  }
}
</script>
