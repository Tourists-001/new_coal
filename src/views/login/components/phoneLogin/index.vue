    ...
    <var-card class="card">
      <template #extra>
        <var-tabs :active.sync="active">
          <var-tab name="msgLogin">短信登录</var-tab>
          <var-tab name="pwdLogin">密码登录</var-tab>
        </var-tabs>
        <var-tabs-items :active.sync="active" can-swipe>
          ...
          >
            ...
            <var-button
              ...
              @click="goCode"
            >
              <var-loading v-if="isPhoneLoading" type="cube" />
        ...
        <div class="sign-in-by-account--btns">
          <span
            class="sign-in-by-account--btns__signup"
        ...
        <div class="sign-in-by-account--third">
          ...
          <span class="text-body1">第三方登录</span>
          <div class="sign-in-by-account--divider" />
        </div>
        <var-button
          text
          block
          class="three-party-login"
          @click="$router.push({ name: 'ScalLogin' })"
        >
...
import { getPhoneCode, PhoneLogin } from '@/api/login'
  ...
  methods: {
    ...
    async goPwdLogin() {
      ...
      try {
        ...
        await PhoneLogin(this.pwdLogin)
      } catch (err) {
        this.isPwdLoading = false
        Snackbar.error('接口错误，请使用二维码登录')
      }
    }
  }
}
