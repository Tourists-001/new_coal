<template>
  <transition name="slide-up">
    ...
    >
      <div
        v-if="(lyricsBackground === 'blur') | (lyricsBackground === 'dynamic')"
        class="lyrics-background"
        :class="{
          'dynamic-background': lyricsBackground === 'dynamic',
        }"
      >
        <div
          class="top-right"
          ...
          <div class="controls">
            <div class="top-part">
              ...
              <div class="buttons">
                ...
                >
                  <svg-icon :icon-class="liked ? 'heart-solid' : 'heart'" />
                </button-icon>
              </div>
            </div>
            <div class="progress-bar">
              ...
              <span>{{ formatTrackTime(currentTrackDuration) }}</span>
            </div>
      ...
      <div class="right-side">
        <transition name="slide-fade">
          ...
          >
            ...
            >
              <div class="lerics_content">
                <span v-if="line.contents[0]">{{ line.contents[0] }}</span>
                <br>
                <span
                  v-if="line.contents[1] && showLyricsTranslation"
                  class="translation"
                >
                  {{ line.contents[1] }}
                </span>
...
import { hasListSource } from '@/utils/playList'
...
export default {
  name: 'Lyrics',
  components: { buttonIcon, VueSlider },
  data() {
    return {
      lyric: [],
      tlyric: [],
  ...
  computed: {
    ...
    theme() {
      return this.lyricsBackground !== true ? 'drak' : 'auto'
    },
    artist() {
    ...
    lyricFontSizes() {
      return {
        fontSize: `${this.lyricFontSize || 28}px`
      }
    },
  ...
  methods: {
    ...mapMutations('main', ['enableScrolling']),
    ...mapMutations('player', ['switchRepeatModes']),
    ...
    async getLyric() {
      ...
      const data = await getLyric(this.currentTrack.id)
      if (!data?.lrc?.lyric) {
        this.lyric = []
        this.tlyric = []
        return false
    ...
    setLyricsInterval() {
      this.lyricsInterval = setInterval(() => {
        const progress = getStore('playerCurrentTrackTime') || 0
    ...
    hasList() {
      return !this.isPersonalFM && this.playlistSourceType.id !== 0
    },
    getListPath() {
      hasListSource()
    },
    ...
    getCoverColor() {
      if (this.lyricsBackground !== true) return
