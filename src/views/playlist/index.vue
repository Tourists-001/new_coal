  ...
  <div v-show="show" class="playlist">
    ...
    >
      ...
      <div class="info">
        ...
        <div class="buttons">
          <ButtonTwoTone icon-class="play" @click.native="playPlaylistByIDs()">
            {{ '播放' }}
          </ButtonTwoTone>
          <ButtonTwoTone
            v-if="playlist.creator.userId !== userId"
            :icon-class="isLikePlayList ? 'heart-solid' : 'heart'"
            :icon-button="true"
    ...
    <div v-if="isLikeSongsPage" class="user-info">
      <h1>
        <img class="avatar" :src="avatar | resizeImage" loading="lazy">
        {{ nickname }}{{ '喜欢的音乐' }}
      </h1>
    ...
    <ContextMenu ref="playlistMenu">
      ...
      <div class="item" @click="likePlaylist">
        {{ isLikePlayList ? '从喜欢列表移出' : '加入喜欢列表' }}
      </div>
...
export default {
  name: 'Playlist',
  components: { Cover, ButtonTwoTone, TrackList, ContextMenu, Modal },
  // 页面加载完成，自动获取焦点
  directives: {
    focus: {
  ...
  computed: {
    ...mapState('player', ['changepalyListLiked']),
    ...
    isLikePlayList() {
      const arr = this.liked?.playlists.includes(this.id)
      console.log(arr)
      return arr
    },
    specialPlaylistInfo() {
    ...
    isUserOwnPlaylist() {
      return this.playlist.creator.userId === this.userId
    }
  },
  created() {
    if (this.$route.name === 'likedSongs') {
      if (getStore('likedSongPlaylistID')) {
        this.getData(getStore('likedSongPlaylistID'))
      }
    } else {
  ...
  methods: {
    ...
    ...mapActions('liked', ['likePlayLists']),
    // 获取数据
    async getData(id, next = undefined) {
      this.id = id
      const data = await getPlaylistDetail(this.id, true)
